...
class AdminController extends Controller {
    ...
    public function add_menus() {
        ...
        if ( ! $is_plesk && $stats && ( $stats->is_free() || $stats->is_exceeded() || $stats->is_free_trial() ) ) {
            // Add upgrade submenu item
            $menu->entry( __( 'Upgrade', 'baqend' ), 'baqend_upgrade', [ $this, 'render_upgrade' ], true );
        }
        ...
        if ( ! $is_plesk && $speed_kit_enabled && $remaining_days > 0 && $remaining_days <= 7 && ! $stats->has_next_plan() ) {
            ...
            <div class="notice notice-warning is-dismissible">
                ...
                <p>
                    ...
                    <?php _e( 'Upgrade your plan to continue gaining the performance boost of Speed Kit for your currently hosted WordPress blog.', 'baqend' ) ?>
                </p>
                <p>
                    <a href="<?php echo baqend_admin_url( 'baqend_upgrade' ); ?>"><?php _e( 'Click here to upgrade your plan', 'baqend' ) ?></a>
                </p>
            </div>
            <?php
        }
        ...
        if ( ! $is_plesk && $is_exceeded && $speed_kit_enabled ) {
            ...
            <div class="notice notice-warning is-dismissible">
                ...
                <p>
                    ...
                    <?php _e( 'Upgrade your plan to gain the performance boost of Speed Kit for your currently hosted WordPress blog.', 'baqend' ) ?>
                </p>
                <p>
                    <a href="<?php echo baqend_admin_url( 'baqend_upgrade' ); ?>"><?php _e( 'Click here to upgrade your plan', 'baqend' ) ?></a>
                </p>
...
//        $stats = new Stats(['appName' => '', 'fixPrice' => 0, 'requestsNow' => 114e3, 'requestsMax' => 150e3, 'priceMax' => 10000, 'priceNow' => 400, 'trafficMax' => 4*1024*1024*1024, 'trafficNow' => 4*1024*1024*1024]);
    ...
    public function render_upgrade() {
        // Generate the upgrade fields
        $this->view
            ->set_template( 'upgrade.php' )
            ->assign( 'app_name', $this->plugin->app_name )
            ->assign( 'stats', $this->stats() )
            ->render();
    }
